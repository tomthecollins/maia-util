...
exports.default = resolve_expressions;
  ...
  for (var j = 0; j < expressions.length; j++) {
    if (expressions[j].type.wedge) {
      ...
      while (k < wedge_stops.length) {
        if (wedge_stops[k].staffNo == expressions[j].staffNo && wedge_stops[k].placement == expressions[j].placement && wedge_stops[k].ontime >= expressions[j].ontime) {
          // We found it!
          target_idx = k;
